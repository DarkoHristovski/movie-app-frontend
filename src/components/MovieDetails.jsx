      ...
      <div className="flex justify-center">
        ...
        <div className="w-2/4">
          <ul className="pt-5 flex flex-wrap w-3/4 justify-between">
            {movie.title && <li className="text-2xl mb-5">Title:{movie.title}</li>}
            {movie.year && (
              <li className="w-2/4 text-2xl mb-5">Director: {movie.director}</li>
            )}
            {movie.year && (
              <li className="w-2/4 text-2xl mb-5">Year: {movie.year}</li>
            )}
            {movie.rating && (
              <li className="w-2/4 text-2xl mb-5">Rating: {movie.rating}/10</li>
            )}
            {movie.length && (
              <li className="text-2xl mb-5">Length: {movie.length} min</li>
            )}
            {movie.language && (
              <li className="w-2/4 text-2xl mb-5">Language: {movie.language}</li>
            )}
            {movie.genre && (
          ...
          <div className="mt-10">
            <Link
              className="bg-gray-300 mt-6 mr-3 hover:bg-gray-400 text-gray-800 font-bold py-2 px-4 rounded"
              to={`/movies/${id}/update`}
            >
              Edit
            </Link>
